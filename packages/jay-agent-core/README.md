...
[![PyPI version](https://badge.fury.io/py/jay-agent-core.svg)](https://badge.fury.io/py/jay-agent-core)
[![Python](https://img.shields.io/pypi/pyversions/jay-agent-core.svg)](https://pypi.org/project/jay-agent-core/)
...
```python
from jay_agent_core import Agent, ProfileManager
from jay_llm import LLM
...
```python
from jay_agent_core.tools import TOOL_SCHEMAS, HANDLERS
from jay_agent_core.tools.registry import ToolRegistry
...
```python
from jay_agent_core.resilience.profile import ProfileManager
...
```python
from jay_agent_core.resilience.retry import resilient_streaming_call
...
```python
from jay_agent_core.observability.events import AgentEvent, AgentEventType
...
jay-agent-core provides protocol-based extension points for customizing agent behavior. All protocols use Python's `Protocol` for structural typing, allowing products to provide custom implementations.
...
```python
from jay_agent_core import MemoryProvider, Message
...
```python
from jay_agent_core import compress_messages, CompressionConfig

# Use built-in 3-level compression
from jay_agent_core.context import compress_messages
...
jay-agent-core includes three core tools:
...
from jay_llm import LLM
from jay_agent_core.resilience.profile import ProfileManager
from jay_agent_core.observability.events import AgentEvent
...
# Run with coverage
pytest tests/ --cov=jay_agent_core --cov-report=html
```
