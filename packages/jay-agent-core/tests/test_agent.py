...
from jay_agent_core.models import AgentState
...
async def test_agent_respond_stream_basic():
    ...
    from jay_llm.models import StreamChunk

    mock_llm = Mock()
    mock_llm.config = Mock(model="test-model")

    async def mock_astream(*args, **kwargs):
        yield StreamChunk(content="Hello world")
    ...
    # _master_loop yields one character at a time
...
async def test_agent_respond_non_streaming():
    ...
    async def mock_astream(*args, **kwargs):
        yield StreamChunk(content="Complete response")
...
async def test_agent_respond_with_cancellation():
    """Test cancellation support — cancel is checked after arun() completes."""
    from jay_llm.models import StreamChunk
    ...
    # cancel is set before first char is yielded, so nothing comes through
    assert chunks == []
