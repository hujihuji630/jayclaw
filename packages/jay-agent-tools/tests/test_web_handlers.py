...
import pytest
from jay_agent_tools.web.handlers import handle_read_webpage, handle_search_web
from jay_agent_tools.web.providers.base import SearchResult
...
async def test_search_web_auto_detects_provider_no_key():
    """Without any API key, auto-detection falls back to FallbackSearchProvider."""
    from jay_agent_tools.web.providers import FallbackSearchProvider

    with patch("jay_agent_tools.web.providers.get_default_provider") as mock_get:
        mock_provider = AsyncMock(spec=FallbackSearchProvider)
...
def _make_reader(content: str, title: str = ""):
    ...
    from jay_agent_tools.web.providers.base import PageContent
