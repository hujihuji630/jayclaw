...
import pytest
from jay_agent_core.tools import _global_registry
...
def test_register_tools_with_global_registry():
    ...
    assert "search_zhihu" in registered
    assert "translate_to_english" in registered
    assert "check_pinyin_naming" in registered
...
def test_register_tools_with_old_registry_raises_error():
    ...
    from jay_agent_core.registry import ToolRegistry as OldToolRegistry
...
def test_handlers_can_be_called_directly():
    ...
    from jay_agent_tools.web import handle_search_web
